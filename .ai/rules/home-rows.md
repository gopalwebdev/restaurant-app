---
paths:
  - 'app/Filament/Admin/Resources/HomeRows/**'
---

# Home Rows

## The home screen is rows in the panel, and tiles live inside a row
HomeRowResource is the storefront navigation item; there is no standalone tiles resource any more (`filament.admin.resources.home-tiles.*` became `...home-rows.*`). Rows are created in a modal on the list and dragged into order there; editing one opens a page, because TilesRelationManager hangs under the form — shaping a row and filling it are the same sitting.
