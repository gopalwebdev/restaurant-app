...
The one that bites: `->reorderable('column')` makes Filament ask for `reorder()`, which is not one of the methods `make:policy` generates. `MenuPolicy`, `MenuCategoryPolicy`, `MenuItemPolicy` and `HomeTilePolicy` all have it — every one of those tables is reorderable, because the order is what the restaurant is arranging. Bulk actions likewise need `deleteAny()`.
