...
The one that bites: `->reorderable('column')` makes Filament ask for `reorder()`, which is not one of the methods `make:policy` generates. `MenuCategoryPolicy` and `MenuItemPolicy` both have it. Bulk actions likewise need `deleteAny()`.
