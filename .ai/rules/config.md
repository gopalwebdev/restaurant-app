...
Queued work needs `php artisan horizon` running; nothing is delivered without it. Supervisor queues are `['mail', 'default']` in that order, and `/horizon` is gated on the product team flag (`User::isSuperAdmin()`) in HorizonServiceProvider.
